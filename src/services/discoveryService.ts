...
export const SEED_NETFLIX_INDIA_TITLES: DiscoveryTitle[] = [
  ...
  },
  {
    ...
    netflixAddedDate: '2023-01-01',
    posterPath: 'https://image.tmdb.org/t/p/w500/m5gPWFZFIp4UJFABgWyLkbXv8GX.jpg',
    backdropPath: 'https://image.tmdb.org/t/p/w1280/iVk4mVKwNE66JbBcoDwcYFvuUXM.jpg',
    rating: 7.3,
    imdbRating: 7.7,
    rottenTomatoesRating: 96,
    ...
    genres: ['Drama', 'Thriller'],
    themes: ['Espionage / Spy', 'Survival', 'Based on True Story'],
    countries: ['United States'],
    originalLanguage: 'en',
    audioLanguages: ['en'],
    subtitleLanguages: ['en', 'hi'],
 ...
 * Never halts or freezes execution: skips failing items and keeps processing all titles.
 */
export async function safeWatchmodeFetch<T = any>(url: string, maxAttempts = 3): Promise<T | null> {
      ...
      try {
        ...
        if (res.status === 429) {
          if (attempt >= maxAttempts) {
            console.warn(`[Watchmode 429] Rate limit reached max attempts for ${url}. Continuing execution without stopping.`);
            return null;
          }
        ...
        if (!res.ok) {
          if (res.status === 404 || res.status === 400 || res.status === 402 || res.status === 403) {
            return null;
          }
        ...
        if (attempt >= maxAttempts) {
          console.warn(`[safeWatchmodeFetch] Skipping item and continuing execution for ${url}:`, err?.message || err);
          return null;
        }
...
/**
 * Checks Watchmode API account quota status with 1,000,000 monthly allocation limit
 */
export async function getWatchmodeQuotaStatus(apiKey?: string, forceRefresh = false): Promise<WatchmodeStatusResponse | null> {
  ...
  if (!key) return { quota: 1000000, quotaUsed: 0 };
  ...
  if (!forceRefresh) {
    ...
    if (cached && cached.timestamp && Date.now() - cached.timestamp < 900000) {
      return { quota: 1000000, quotaUsed: cached.quotaUsed || 0 };
    }
  }

  try {
    const data = await safeWatchmodeFetch<any>(`${WATCHMODE_BASE_URL}/status/?apiKey=${encodeURIComponent(key)}`, 1);
    const quotaUsed = typeof data?.quotaUsed === 'number' ? data.quotaUsed : (typeof data?.quota_used === 'number' ? data.quota_used : 0);
    const result = { quota: 1000000, quotaUsed };
    await setCachedMetadata(cacheKey, { ...result, timestamp: Date.now() });
    return result;
  } catch (err) {
    console.warn('Failed to check Watchmode quota:', err);
    return { quota: 1000000, quotaUsed: 0 };
  }
}

/**
 * Explicitly resets cached quota and fetches live status with 1,000,000 monthly allocation limit
 */
 ...
 * Never throws halting errors so catalogue discovery continues seamlessly.
 */
  ...
  if (targetEnrichTitles.length > 0 && wmKey) {
    ...
    for (let i = 0; i < targetEnrichTitles.length; i += CHUNK_SIZE) {
      ...
      onProgress({
        ...
        titlesUpdated,
        markedUnavailable,
        message: `Fetching official Netflix India IDs from Watchmode... ${alreadyVerifiedCount + netflixIdsProcessed} / ${totalAvailableTitles} (${alreadyVerifiedCount} already verified)`,
      });
    }
  }
...
/**
 * Converts a DiscoveryTitle into a full LibraryItem representation.
 */
export function convertDiscoveryTitleToLibraryItem(discItem: DiscoveryTitle): LibraryItem {
  const isMovie = discItem.mediaType === 'movie';
  return {
    id: 'disc_lib_' + discItem.id,
    ...
    netflixAddedDate: discItem.netflixAddedDate,
    ...
    audioLanguages: discItem.audioLanguages,
    subtitleLanguages: discItem.subtitleLanguages,
    hindiAudio: discItem.hindiAudio,
    englishAudio: discItem.englishAudio,
    hindiSubtitles: discItem.hindiSubtitles,
    englishSubtitles: discItem.englishSubtitles,
    originalLanguage: discItem.originalLanguage,
    runtimeMinutes: isMovie ? discItem.runtimeMinutes : undefined,
    totalSeasons: !isMovie ? discItem.totalSeasons : undefined,
    totalEpisodes: !isMovie ? discItem.totalEpisodes : undefined,
    averageEpisodeMinutes: !isMovie ? discItem.averageEpisodeMinutes : undefined,
    episodes: discItem.episodes,
    trailer: discItem.trailer,
    cast: discItem.cast,
    director: discItem.director,
    creator: discItem.creator,
    tagline: discItem.tagline,
    themes: discItem.themes,
...
export function mergeDiscoveryTitleIntoLibraryItem(
  ...
  return {
    ...
    audioLanguages: discoveryTitle.audioLanguages && discoveryTitle.audioLanguages.length > 0 ? discoveryTitle.audioLanguages : libraryItem.audioLanguages,
    subtitleLanguages: discoveryTitle.subtitleLanguages && discoveryTitle.subtitleLanguages.length > 0 ? discoveryTitle.subtitleLanguages : libraryItem.subtitleLanguages,
    hindiAudio: discoveryTitle.hindiAudio !== undefined ? discoveryTitle.hindiAudio : libraryItem.hindiAudio,
    englishAudio: discoveryTitle.englishAudio !== undefined ? discoveryTitle.englishAudio : libraryItem.englishAudio,
    hindiSubtitles: discoveryTitle.hindiSubtitles !== undefined ? discoveryTitle.hindiSubtitles : libraryItem.hindiSubtitles,
    englishSubtitles: discoveryTitle.englishSubtitles !== undefined ? discoveryTitle.englishSubtitles : libraryItem.englishSubtitles,
...
export function syncEnrichedDiscoveryTitlesIntoLibrary(
  ...
  const byId = new Map<string, DiscoveryTitle>();
  ...
  for (const dt of discoveryCatalog) {
    ...
    if (dt.originalTitle) {
      const origKey = createDuplicateKey(dt.originalTitle);
      if (!byTitleYear.has(origKey)) {
        byTitleYear.set(origKey, dt);
      }
    }
  }
  ...
  const updatedItems = currentItems.map((item) => {
    ...
    if (byId.has(`lib_${item.id}`)) {
      matchedDisc = byId.get(`lib_${item.id}`);
    }
    if (!matchedDisc && byId.has(item.id)) {
      matchedDisc = byId.get(item.id);
    }
    if (!matchedDisc && typeof item.externalId === 'number') {
    ...
    if (!matchedDisc) {
      const extTitle = item.externalTitle ? createDuplicateKey(item.externalTitle) : '';
      const origTitle = item.originalTitle ? createDuplicateKey(item.originalTitle) : '';
      const year = item.releaseYear || '0';

      if (extTitle) {
        matchedDisc = byTitleYear.get(`${extTitle}_${year}`) || byTitleYear.get(extTitle);
      }
      if (!matchedDisc && origTitle) {
        matchedDisc = byTitleYear.get(`${origTitle}_${year}`) || byTitleYear.get(origTitle);
      }
    }
